...
step initGof = let  survives cell gof =
                    ...
                    survived    = Set.foldl (\cell gof' -> if survives cell initGof then insert cell gof' else gof') empty initGof
                    newborn     = Set.foldl (\cell gof' -> if (countNghb cell initGof) == 3 then insert cell gof' else gof') empty emptyCells 
